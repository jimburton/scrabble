...
From now on, the `playGame`, `takeTurn`, `takeTurnManual` and
`takeTurnAI` functions are very similar to the versions from the CLI
client. The biggest difference is of course that non-AI clients
interact with the server using JSON. In the `takeTurnManual` function
a message is read from the client connection and decoded. There
follows a case statement that pattern matches the message. If the
message can't be decoded or isn't one of the messages the server is
expecting, it loops back to `takeTurn`. Otherwise, the message is a
request for hints or one of the ways of playing a move.
...
The client is written as a small web application, with most of the
effort going in to the JavaScript. I'm not going to go into any detail
about the code, as JavaScript techniques aren't the subject of this book.
Suffice to say, the event handler that receives incoming messages from the 
server has a `switch` statement (the equivalent of Haskell's `case` 
statement) that pattern matches on messages in a very similar way to the one
in `takeTurnManual` above, except that it is expecting a different set of 
messages. 

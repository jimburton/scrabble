...
Now that we can place words onto a board we want to check that such
moves are valid. The rules about word placement are as follows:
...
message explaining what went wrong. A common solution to writing a function that
returns a value of type `a` or an error message is to use the type
`Either Text a`. Values of this type are either `Left e`, where `e` is
an error message, or `Right x` where `x :: a`. In the case of
`straight`, if it returns any type of `Right` value then we know
things went well. So there's no need to return a `Bool`, we can just
return `Left` for an error or `Right ()` if things went well. `()`
("unit") is the type with exactly one value in it (which is also
`()`):
...
using `dictContainsWord`. We need to apply this function to the new
